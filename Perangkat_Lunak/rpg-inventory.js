...
let handler = async (m, { conn }) => {
  ...
  let isMods = [conn.decodeJid(global.conn.user.id), ...global.owner.filter(([number, _, isDeveloper]) => number && isDeveloper).map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(who)
...
`.trim()
    conn.adReply(m.chat, caption, 'I N V E N T O R Y', '', flaImg.getRandom() + 'INVENTORY', setting.namabot, m)
}
