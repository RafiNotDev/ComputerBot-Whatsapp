  ...
  let response = await conn.query({
    ...
    for (const user of participant.filter(item => item.attrs.error == 403)) {
        ...
        await conn.sendMessage(text + '@s.whatsapp.net', {
         ...
         caption: `Masuk Ke Group Aku Yok Kakak\n\n*Link Group:* ${await conn.getName(m.chat)}\n\nhttps://chat.whatsapp.com/` + await conn.groupInviteCode(m.chat) + `\n\n${setting.namabot}`}, {quoted: m})
m.reply('Link Group Sudah Terkirim Ke Nomor Target')}
} catch (e) {
await conn.sendMessage(text + '@s.whatsapp.net', {
         image: {url: setting.tampilan},
         caption: `Masuk Ke Group Aku Yok Kakak\n\n*Link Group:* ${await conn.getName(m.chat)}\n\nhttps://chat.whatsapp.com/` + await conn.groupInviteCode(m.chat) + `\n\n${setting.namabot}`}, {quoted: m})
m.reply('Link Group Sudah Terkirim Ke Nomor Targettt')
}
}
