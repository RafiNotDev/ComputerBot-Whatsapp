...
let handler = async (m, { conn, args, __dirname, usedPrefix, command }) => {
    try {
        ...
        if (/audio/.test(mime)) {
            ...
            let filename = join(__dirname, `../Cache/Sampah/${getRandom('.mp3')}`)
            let media = await q.download(true)
            exec(`ffmpeg -i ${media} ${set} ${filename}`, async (err, stderr, stdout) => {
                unlinkSync(media)
                if (err) return m.reply(`_*Error!*_`)
                let buff = readFileSync(filename)
                await conn.sendFile(m.chat, buff, filename, false, m, true, { mimetype: 'audio/mpeg' })
            })
        } else {
            rafitampilan(m.chat, `[ *Penggunaan Salah* ]\n${usedPrefix + command} Sambil Replay Audio Atau Voice Note`)
        }
    } finally {
 await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key }})}
}
