    ...
    let sock = Object.defineProperties(conn, {
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                ...
                if (data && saveToFile && !filename) (filename = path.join(__dirname, '../../Cache/Sampah/' + new Date * 1 + '.' + type.ext), await fs.promises.writeFile(filename, data))
                return {
                    res,
                    filename,
                    ...type,
...
`.trim()
        ...
        preSudo: {
            async value(text, who, m, chatupdate) {
                ...
                messages.key.fromMe = areJidsSameUser(who, conn.user.jid)
        ...
        cMod: {
            ...
            value(jid, message, text = '', sender = conn.user.jid, options = {}) {
                ...
                copy.key.fromMe = areJidsSameUser(sender, conn.user.jid) || false
                ...
                for (const message of m) {
                    try {
                        ...
                        if (isGroup) {
                            ...
                            sender = conn.decodeJid(message.key?.fromMe && conn.user.jid || message.participant || message.key?.participant || chat || '')
                            if (sender !== chat) {
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                return this.conn?.decodeJid(this.key?.fromMe && this.conn?.user.jid || this.participant || this.key.participant || this.chat || '')
            },
            enumerable: true
        },
        fromMe: {
            get() {
                return this.key?.fromMe || areJidsSameUser(this.conn?.user.jid, this.sender) || false
            }
        },
        mtype: {
            get() {
