...
const Sampah = path.join(__dirname, '../../Cache/Sampah')
/**
 * Image to Sticker
...
function sticker2(img, url) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let inp = path.join(Sampah, +new Date + '.jpeg')
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    const Sampah = path.join(__dirname, `../../Cache/Sampah/${+ new Date()}.${type.ext}`)
    const out = path.join(Sampah + '.webp')
    await fs.promises.writeFile(Sampah, img)
    // https://github.com/MhankBarBar/termux-wabot/blob/main/index.js#L313#L368
    let Fffmpeg = /video/i.test(type.mime) ? fluent_ffmpeg(Sampah).inputFormat(type.ext) : fluent_ffmpeg(Sampah).input(Sampah)
    Fffmpeg
      .on('error', function (err) {
        console.error(err)
        fs.promises.unlink(Sampah)
        reject(img)
      })
